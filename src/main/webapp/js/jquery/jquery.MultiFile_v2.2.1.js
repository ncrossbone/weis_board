...
/*# AVOID COLLISIONS #*/
	...
	$.fn.MultiFile = function(options){
		...
		//### http://plugins.jquery.com/node/1363a
		...
		.each(function(){
			...
			MultiFile.addSlave = function( slave, slave_count ){
				...
				slave_count= $(MultiFile.list).children().length;
				//alert("MultiFile.list.length:"+ $(MultiFile.list).children().length)
				...
				childrenLen = $(MultiFile.list).children().length
				...
				if ((MultiFile.max-childrenLen) <1) {
					slave.disabled = true;
				}
				...
				$(slave).jfilestyle({'input': false, 'theme': 'image', 'buttonText' : '파일선택'});
				...
				slave.change(function(){
					...
					if(ERROR.length > 0){
						...
						$(this).closest('.fileWrap').find('.jquery-filestyle').css({ position:'absolute', top: '-3000px' });
			...
			MultiFile.addToList = function( slave, slave_count, files ){
				...
				//yhj add - 2013.10.23
				var fileName = MultiFile.STRING.file.replace('$file', v.match(/[^\/\\]+$/gi)[0]);
				...
				$(MultiFile.list).children().each(function(){

					if($(this).text().trim() == fileName.trim()){
						alert(MultiFile.STRING.duplicate.replace('$file', v.match(/[^\/\\]+$/gi)));
				...
				// yhj add - 2013.10.23 end

				// HM add - 2016.08.22 start
				
				var names = $('<div class="MultiFile-label"/>');
				names.css({'width':'120px','height':'150px','float':'left','margin':'10px'});	
				$.each(files, function (i, file) {
					var v = String(file.name || '' ),
							S = MultiFile.STRING,
							n = S.label || S.file || S.name,
							t = S.title || S.tooltip || S.selected,
							p = '<img class="MultiFile-preview" style="'+ MultiFile.previewCss+' width:100px; height:80px;"/>',
							label =	$(
									(
				...
				if(addFlag) {
					var
					a = $('<span class="MultiFile-title" title="'+MultiFile.STRING.selected.replace('$file', v)+'">'+fileName+'.'+ext+'</span>'),
					c = $('<span class="MultiFile-export" style="display:none">'+MultiFile.STRING.text+'</span>'),
					b = $('<br><span class="MultiFile-remove"><a href="#'+MultiFile.wrapID+'" class="icn_fileDel">'+MultiFile.STRING.remove+'</a></span>');
					...
					//	$(MultiFile.current).css({ position:'', top: '' });
						...
						//$(MultiFile.current).jfilestyle('clear');
	...
	*/
	...
	reset: function(){
		var settings = $(this).data('MultiFile');
			...
			if(settings) {
				settings.list.find('span.MultiFile-remove').click();
			}
	return $(this);
	},
