...
public class EgovFormBasedUUID implements Serializable
{
	...
	public static EgovFormBasedUUID nameUUIDFromBytes(byte[] name)
	{
		...
		try
		{
			// 2011.10.10 보안점검 후속조치 암.호.화. 알고리즘 변경(MD5 -> SHA-256)
		...
		byte[] sha256Bytes = new byte[8];
		System.arraycopy(sha, 0, sha256Bytes, 0, 8);
		...
		return new EgovFormBasedUUID(sha256Bytes);
	}
