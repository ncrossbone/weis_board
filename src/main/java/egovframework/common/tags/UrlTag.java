...
package egovframework.common.tags;
...
public class UrlTag extends HtmlEscapingAwareTag implements ParamAware {
	...
	public void setValue(String value) {
		...
		// ���� ���ڿ��� ���� URL�� �����Ѵ�.
			...
			this.value = (String) request.getAttribute("javax.servlet.forward.request_uri");
			if( this.value == null )
				this.value = request.getRequestURI();
			
			// GET���� �Ѿ�� ������Ʈ�� this.params �� �ֱ�. - �Ϸ��ߴµ� doStartTagInternal �޼��尡 ���߿� ȣ��ȴ�. �ڵ庹�� -_-
			
			/*
			// ��� �Ķ���Ϳ� ���� GET/POST üũ�ؼ� �Ϸ��ߴµ� �켱 ����.
			...
			while( paramEnum.hasMoreElements() ) {
				...
				param.setValue( request.getParameter(paramName) );
	...
	public void addParam(Param param) {
		...
		*/
		...
		HttpServletRequest request = (HttpServletRequest) pageContext.getRequest();
		String mt = request.getMethod().toUpperCase();
		if(mt.equals("POST")){
		  Enumeration paramEnum = request.getParameterNames();
	    while( paramEnum.hasMoreElements() ) {
	      ...
	      param2.setValue( request.getParameter(paramName) );
	...
	public int doStartTagInternal() throws JspException {
		//this.params = new LinkedList<Param>();
		this.params = extractQueryString( (HttpServletRequest) pageContext.getRequest() );
	...
	/**
	 * org.apache.struts2.views.jsp.URLTag ���� �Ϻ� ����. �켱 ������ ����.
	 * @param request
	 * @return
	 * @see 
	 */
	private List<Param> extractQueryString( HttpServletRequest request ) {
	  
	  if(request.getMethod().toUpperCase().equals("POST")) return new LinkedList<Param>();
    ...
    for(int i = 0; i < queryArray.length; i++ ) {
    	String[] paramArray = queryArray[i].split( "=" );
    	
    	if( paramArray.length > 1 &&  paramArray[1] != null && !paramArray[1].equals( "" ) ) {
    		Param param = new Param();
    		param.setName( paramArray[0] );
    		
    		//TODO: URL ���鶧 �ٽ� ���ڵ� �Ǳ� ������, ���ڵ��ؼ� �־�� ��. �̷� ����.. ���߿� ������ ��! 
    		//param.setValue( paramArray[1] );
    		String paramValue = paramArray[1];
    		try {
	        paramValue = URLDecoder.decode( paramValue, pageContext.getRequest().getCharacterEncoding() );
        }
