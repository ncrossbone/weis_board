			...
			success: function (data) {
				...
				if(data != '' && data != null) {
					...
					data.forEach(function (item) {
						if(rtnAdm_cd.indexOf(item.BOOBSCD) > -1){
							checked = "checked";
						}else{
							checked = "";
						}
						...
						selOpt += "   <td><input name='chkSite' id='chkSite' type='checkbox' value='" + item.BOOBSCD + "' class='item'"+checked+"/></td>";
    ...
    <table class="st01" summary="보 운영현황으로 이루어진 표">
		...
		<thead>
            <tr>
				<th>보 코드</th>
				<th>보 지점</th>
                <th>관측일시<span>(분)</span></th>
                <th>보 상류수위<span>(m)</span></th>
                <th>보 하류수위<span>(m)</span></th>
                <th>저수량<span>(백만m^3)</span></th>
                <th>공용량<span>(백만m^4)</span></th>
                <th>유입량<span>(백만m^5)</span></th>
                <th>총 방류량<span>(㎥/sec)</span></th>
                <th>발전 방류량<span>(㎥/sec)</span></th>
                <th>가동보 방류량<span>(㎥/sec)</span></th>
                <th>고정보 방류량<span>(㎥/sec)</span></th>
                <th>어도 방류량<span>(㎥/sec)</span></th>
                <th>기타 방류량<span>(㎥/sec)</span></th>
		...
		<tbody id="resultSiteTable">
			...
			<c:if test="${not empty resultList}">
				<c:forEach var="item" items="${resultList}" varStatus="idx">
					<tr>
						<td><c:out value="${item.BOOBSCD}"/></td>
						<td><c:out value="${item.SBSNCD}"/></td>
						<td><c:out value="${item.YMDHM}"/></td>
						<td><c:out value="${item.SWL}"/></td>
						<td><c:out value="${item.OWL}"/></td>
						<td><c:out value="${item.SFW}"/></td>
						<td><c:out value="${item.ECPC}"/></td>
						<td><c:out value="${item.INF}"/></td>
						<td><c:out value="${item.TOTOTF}"/></td>
						<td><c:out value="${item.EGOTF}"/></td>
						<td><c:out value="${item.GTOTF}"/></td>
						<td><c:out value="${item.CBOTF}"/></td>
						<td><c:out value="${item.FWOTF}"/></td>
						<td><c:out value="${item.ETCOTF}"/></td>
