package egovframework.common.jfile.taglibs;
...
public class FileUploadTag extends TagSupport
{
	...
	/** 썸네일 이미지 생성 여부 */
	private String fileThumbnailYn;
	...
	public int doStartTag() throws JspException
	{
		try
		{
			...
			.append(getObjectId() + "= new jwork.fileUpload( \"" + getObjectId() + "\", \"" + request.getContextPath() + "\", \"" + getFileId() + "\", {maxFileSize: " + getMaxFileSize() + ", uploadCompleted:" + getUploadCompletedEvent() + ", beanId:\"" + getBeanId() + "\", fileType:\"" + getFileType() + "\", uploadMode:\"" + getUploadMode() + "\", maxFileCount:" + getMaxFileCount() + ", usePreview:" + getUsePreview() + ", useSecurity:" + getUseSecurity() + ", fileThumbnailYn:\"" + getFileThumbnailYn() + "\"});")
			.append("});")
			.append("</script>")
	...
	/**
	 * 보안 적용 암.호.화. 적용 여부 값을 반환
	 * 
	 * @return String 보안 적용 암.호.화. 적용 여부 값
	 */
