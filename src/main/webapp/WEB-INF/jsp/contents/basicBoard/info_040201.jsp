...
<script type="text/javascript">
	...
	function searchSite(){
		...
		var param = { ADM_CD:admCd, SEARCH_TYPE:_searchType, DO_TYPE:'boobsif' };
	...
	function searchWaterSys(){
		...
		var param = { WS_CD:getWsCd, AM_CD:getAmCd, SEARCH_TYPE:_searchType, DO_TYPE:'boobsif' };
	...
	function searchName(){
        ...
        _searchType = "5";
        var param = { ST_NM:getName, SEARCH_TYPE:_searchType, DO_TYPE:'boobsif' };
...
<form name="resultFrm" id="resultFrm" method="post">
	...
	<input type="hidden" id="mId" name="mId" value ="ExcelsedimentEval"/>
...
<div class="result sc">
    <table class="st01" summary="측정소명, 주소, 조사기관으로 이루어진 표">
		...
		<thead>
            <tr>
				<th rowspan="2">보 명</th>
				<th rowspan="2">지점 구분</th>
				<th colspan="5">채취시기/회차</th>
				<th colspan="6">현장측정값</th>
				<th colspan="5">퇴적물용출률 측정값</th>
            ...
            <tr>
				...
				<th>현장_SOD(g/m2/d)</th>
				<th>폭기_SOD(g/m2/d)</th>
				<th>BNF_NH4-N(g/m2/d)</th>
				<th>BNF_NOx-N(g/m2/d)</th>
				<th>BNF_PO4-P(g/m2/d)</th>
            </tr>
		</thead>
		<tbody>
			...
			<c:if test="${not empty resultList}">
				<c:forEach var="item" items="${resultList}" varStatus="idx">
					<tr>
						...
						<td>${item.EXAMIN_MT }</td>
						<td>${item.EXAMIN_DE }</td>
						<td>${item.EXAMIN_TIME }</td>
						<td>${item.SURLYR_DPWT }</td>
						<td>${item.SURLYR_WTRTP }</td>
						<td>${item.SURLYR_DOXY }</td>
						<td>${item.BOTLYR_DPWT }</td>
						<td>${item.BOTLYR_WTRTP }</td>
						<td>${item.BOTLYR_DOXY }</td>
						<td>${item.SPT_SOD }</td>
						<td>${item.AERT_SOD }</td>
						<td>${item.BNF_NH4N }</td>
						<td>${item.BNF_NOXN_CM }</td>
						<td>${item.BNF_PO4P }</td>
						<td>${item.PARTCLR_MATTER }</td>
					</tr>
                </c:forEach>
			</c:if>
		</tbody>
